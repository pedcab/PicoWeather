...
if previousTimeLogRead == "": #evaluate if file is empty 
   previousTimeLog=1 #give a value of 1 second if file is empty (to avoid crash trying to read an empty file)
else:
    previousTimeLog= previousTimeLogRead #if file not empty attribute its stored value to "previous log" variable
file.close()

#LOG SAVE INTERVAL
logSaveInterval = 60 #Time period in seconds between log saves 
...
while (True):
    ...
    if (totalTimeLog - elapsedTime) < (logSaveInterval + 5) && (totalTimeLog - elapsedTime) > (logSaveInterval - 5)
